...
/* ══════════════════════════════════════════
   SOFT SKILLS — CARRUSEL HORIZONTAL
   ══════════════════════════════════════════ */
...
(function () {
  ...
  const cards = Array.from(track.querySelectorAll('.soft-card-tw'));
  ...
  function goTo(idx) {
    ...
    const gap = 24;
    const scrollPos = current * (cardWidth + gap);
  ...
  track.addEventListener('scroll', () => {
    ...
    if (newIndex !== current && newIndex >= 0 && newIndex <= getMaxIndex()) {
      current = newIndex;
      dotsWrap.querySelectorAll('.c-dot').forEach((d, i) => {
        d.classList.toggle('active', i === current);
      });
